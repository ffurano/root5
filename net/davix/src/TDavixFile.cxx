...
struct TDavixFileInternal{
    ...
    inline void setS3Auth(const std::string & key, const std::string & token){
        if(gDebug > 1)
    ...
    inline void parseParams(Option_t* option) {
        // intput params
    ...
    // DAVIX
    Context *davixContext;
    RequestParams *davixParam;
    DavPosix *davixPosix;
    Davix_fd *davixFd;
    TUrl &fUrl;
    Option_t* opt;
...
Int_t TDavixFile::DavixStat(struct stat *st) const {
    DavixError *davixErr = NULL;
    if (d_ptr->davixPosix->stat(d_ptr->davixParam, fUrl.GetUrl(), st, &davixErr) < 0) {
...
/////////////////////////////////////////////////////////
/// \brief TDavixSystem::TDavixSystem
////////////////////////////////////////////////////////
TDavixSystem::TDavixSystem() : TSystem("-Davix", "Davix Helper System")
{
   SetName("Davix");
}
